(function(){
  ...
  var form = { name:'', phone:'', service:'', notes:'' };
  ...
  function renderForm(){
    ...
    setTimeout(function(){
      ...
      ['name','phone','service','notes'].forEach(function(k){
  ...
  function renderDone(){
    ...
    setTimeout(function(){ var r=document.getElementById('bkReset'); if(r) r.onclick=function(){ selected=null; slot=null; form={name:'',phone:'',service:window.SERVICES[0]||'',notes:''}; step=1; render(); }; },0);
    return d;
  }
  function submit(){
    ...
    fetch('/api/book.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({date:selected,slot:slot,name:form.name,phone:form.phone,service:form.service,notes:form.notes})})
